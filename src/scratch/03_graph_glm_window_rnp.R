...
path <- paste("data/processed/GEA/glms/glms_window_summary/glms_window_chunk_analysis_ph_mean_real_nSNP100/")
file_names = as.list(dir(path = path, pattern = "glm_window_chunks_*"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0(paste("data/processed/GEA/glms/glms_window_summary/glms_window_chunk_analysis_ph_mean_real_nSNP100/"), x))))
...
# Graph rnp geompoint
pdf("output/figures/GEA/glms/glms_window_summary/glm_window_rnp_0.01_geompoint_nSNP100.pdf", width = 8, height = 8)
...
# Graph with the permutations
#pdf("output/figures/GEA/glms/glms_window_summary/glm_window_rnp_0.01_geompoint.pdf", width = 8, height = 8)
#ggplot(win.out, aes(y=-log10(rnp.binom.p.0.01), x=chr.unique, col=factor(perm))) + 
#  geom_point(alpha=0.8, size=1.3) + 
...
# Graph rnp geomline
pdf("output/figures/GEA/glms/glms_window_summary/glm_window_rnp_0.01_geomline_nSNP100.pdf", width = 8, height = 8)
ggplot(win.out, aes(y=-log10(rnp.binom.p.0.01), x=chr.unique)) + 
  geom_line( ) + 
  facet_wrap(~variable) +
  theme_bw() + theme(legend.position = "none")
