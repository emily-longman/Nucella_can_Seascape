...
# NOTE THIS IS OVERWRITING THE OTHER FINAL - WHY THIS IS A SEPARATE SCRIPT
bio_oracle_sites_2010_sub <- bio_oracle_sites_2010_sub[,c(1,5,8)]
bio_oracle_sites_ssp585_2090_sub <- bio_oracle_sites_ssp585_2090_sub[,c(1,5,8)]
...
mean(check_rsq$rsq)*100 #13.64572
range(check_rsq$rsq)*100 # 0.001245134 65.344726285
...
# Graph offset
pdf("output/figures/genomic_offset/genomic_offset.pdf", width = 12, height = 4)
...
# Graph offset
pdf("output/figures/genomic_offset/genomic_offset_viridis.pdf", width = 12, height = 4)
ggplot(df_long, aes(x = Site, y = Value, fill = Site)) + geom_col() +  
facet_wrap(~ Variable, scales = "free_y") +
...
df_diff_squared$sum_all_columns <- rowSums(df_diff_squared[1:3])
...
env_df_sub <- env_df %>% dplyr::select(c("chl_mean", "ph_mean", "thetao_max"))
env_df_fut_sub <- env_df_fut %>% dplyr::select(c("chl_mean", "ph_mean", "thetao_max"))
...
genOffset <- sqrt((predRefindfut[,1]-predRefind[,1])^2+(predRefindfut[,2]-predRefind[,2])^2
                     +(predRefindfut[,3]-predRefind[,3])^2)
...
pdf("output/figures/genomic_offset/genomic_offset_ggplot2_geomtile.pdf", width = 8, height = 6)
