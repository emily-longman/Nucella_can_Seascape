...
annotation <- foreach(i=1:dim(bf.Mtross.mean.sum.outliers)[1], .combine = "rbind", .errorhandling = "remove")%do%{

  message(i)
  # Reset filter
  seqResetFilter(genofile)
  # Extract SNP_id for SNP i
  tmp.i = bf.Mtross.mean.sum.outliers[i,]$SNP_id
...
bf.Mtross.mean.sum.outliers.annotated <- left_join(bf.Mtross.mean.sum.outliers, annotation, by = join_by(SNP_id))

# Write output
write.csv(bf.Mtross.mean.sum.outliers.annotated, "data/processed/baypass/bf.Mtross.mean.sum.outliers.annotated.csv", row.names = F, quote = F)
