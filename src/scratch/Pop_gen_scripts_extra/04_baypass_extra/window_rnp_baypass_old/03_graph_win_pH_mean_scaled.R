...
path <- paste("data/processed/baypass/window_summary/window_100kb_chunk_analysis_ph_mean_scaled/")
file_names = as.list(dir(path = path, pattern = "window_chunks_*"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0(paste("data/processed/baypass/window_summary/window_100kb_chunk_analysis_ph_mean_scaled/"), x))))
...
# Save merged data
save(win.out.order, file = "data/processed/baypass/window_summary/window_analysis_ph_mean_scaled.RData")
load("data/processed/baypass/window_summary/window_analysis_ph_mean_scaled.RData")
...
my_ecdf <- ecdf(bf.ph.scaled.mean.sum$bf_db.mean)
...
# Graph rnp geompoint
pdf("output/figures/baypass/window_summary/baypass_window_ph_mean_scaled_rnpPOD_geompoint.pdf", width = 12, height = 6)
...
# Graph rnp geomline
pdf("output/figures/baypass/window_summary/baypass_window_ph_mean_scaled_rnpPOD_geomline_wider.pdf", width = 12, height = 3)
...
# Graph Windows
pdf("output/figures/baypass/window_summary/baypass_window_ph_mean_scaled_rnpPOD_geomline_bywindows.pdf", width = 12, height = 3)
...
# Save outliers
write.csv(win.out.order.outliers, "data/processed/baypass/window_summary/window_100kb_analysis_ph_mean_scaled_outliers.csv", row.names=FALSE)

# ================================================================================== #

# Identify SNPs in the outlier windows -> 45,068 SNPs
outlier.win.SNPs <- foreach(win.i=unique(win.out.order.outliers$win), .combine="rbind", .errorhandling="remove")%do%{
    ...
    baypass.tmp <- bf.ph.scaled.mean.sum %>% filter(
        bf.ph.scaled.mean.sum$chr == win.tmp$chr & 
        bf.ph.scaled.mean.sum$pos >= win.tmp$pos_min &
        bf.ph.scaled.mean.sum$pos <= win.tmp$pos_max)
}
...
# Load windows
win.out.order.outliers <- read.csv("data/processed/baypass/window_summary/window_100kb_analysis_ph_mean_scaled_outliers.csv", header=T)
...
pdf("output/figures/baypass/window_summary/baypass_ph_mean_scaled_BF_topwin.pdf", width = 8.25, height = 4.5)
...
pdf("output/figures/baypass/window_summary/baypass_ph_mean_scaled_BF_topwin_wider.pdf", width = 10, height = 4.5)
ggplot(outlier.win.SNPs.top.win, aes(y=bf_db.mean, x=pos/1000)) + labs(x="Position (kb)", y="BF") +
  geom_rect(aes(xmin=1/1000, xmax=58041/1000, ymin=-Inf, ymax=Inf), fill="grey", alpha=0.5) +
  geom_point(alpha=0.8, size=3.5, aes(colour = cut(bf_db.mean, c(-Inf, 20, Inf)))) + 
  scale_color_manual(values = c("(-Inf,20]" = "black", "(20, Inf]" = "blue")) +
