...
ph.mean.outliers <- read.csv("data/processed/outlier_analyses/ph.mean.outliers.csv", header=T)

#--------------------------------------------------------------------------------

# Extract annotation data for each SNP of interest

annotation <- foreach(i=1:dim(ph.mean.outliers)[1], .combine = "rbind", .errorhandling = "remove")%do%{

  message(i)
  # Reset filter
  seqResetFilter(genofile)
  # Extract SNP_id for SNP i
  tmp.i = ph.mean.outliers[i,]$SNP_id
...
ph.mean.annotated.SNPs <- left_join(ph.mean.outliers, annotation, by = join_by(SNP_id))

# Write output
write.csv(ph.mean.annotated.SNPs, "data/processed/outlier_analyses/ph.mean.annotated.SNPs.csv")
