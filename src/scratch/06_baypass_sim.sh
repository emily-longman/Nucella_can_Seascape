...
else echo "Working POD folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/GEA/baypass/POD; date
fi

#--------------------------------------------------------------------------------

# Change directory 
cd $WORKING_FOLDER/GEA/baypass/POD

# Run baypass on POD
$baypass -npop 19 \
-gfile $WORKING_FOLDER/GEA/baypass/G.NC.baypass.sim \
