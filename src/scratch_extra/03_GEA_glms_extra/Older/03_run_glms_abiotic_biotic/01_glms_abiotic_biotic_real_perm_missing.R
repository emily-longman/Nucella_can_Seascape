...
glm.model.output =
 ...
 ###############################################################
    ...
    # Run model
        ...
        # Shuffle 
        af_i_snp_enviro <- af_i_snp_enviro %>% mutate(shuffle_ph_mean = sample(ph_mean), shuffle_mean_integrated_thk = sample(mean_integrated_thk))

        # Model allele freq
        # Generate 3 models - a null model (t0), a model with just demography (t1.dem) and a model with demography and "j" enviro var (t1.dem.env)
        y.perm <- af_i_snp_enviro$af_nEff
        X.null.perm <- model.matrix(~1, af_i_snp_enviro)
        X.dem.perm <- model.matrix(~PC1, af_i_snp_enviro)
        X.dem.abiotic.perm <- model.matrix(~PC1+shuffle_ph_mean, af_i_snp_enviro)
        ...
        data.frame(
          ...
          AIC_dem_abiotic = c(AIC(t1.dem.abiotic.perm)),
          AIC_dem_biotic = c(AIC(t1.dem.biotic.perm)),
          AIC_dem_both = c(AIC(t1.dem.both.perm)),
          AIC_dem_both_int = c(AIC(t1.dem.both.int.perm)),
          b_int = last(t1.dem.both.int.perm$coef),
          se_int = last(t1.dem.both.int.perm$se),
