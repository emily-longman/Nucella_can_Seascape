...
# Subset real data
glm.real.chunk <- glm.real %>% filter(SNP_id %in% missing_SNP_ids)
# Subset perm data
glm.perm.chunk <- glm.perm %>% filter(SNP_id %in% missing_SNP_ids)
...
file_name <- paste("GLM_output_chunk_missing", sep = "")
