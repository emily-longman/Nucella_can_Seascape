...
if [ -d "window_100kb_chunk_analysis_Mtross_mean" ]
then echo "Working window_100kb_chunk_analysis_Mtross_mean folder exist"; echo "Let's move on."; date
else echo "Working window_100kb_chunk_analysis_Mtross_mean folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/baypass/window_summary/window_100kb_chunk_analysis_Mtross_mean; date
fi

#--------------------------------------------------------------------------------

# Run R script
Rscript --vanilla $WORKING_FOLDER/src/04_baypass/09_window_rnp_100kb/02_baypass_window_rnp_Mtross_mean.R "${SLURM_ARRAY_TASK_ID}"
