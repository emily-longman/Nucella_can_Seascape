...
# Name this job
#SBATCH --job-name=ph_slim_signmoid
...
# This part of the script will check and generate, if necessary, all of the output folders used in the script
if [ -d "ph_results_sigmoid" ]
then echo "Working ph_results_sigmoid folder exist"; echo "Let's move on."; date
else echo "Working ph_results_sigmoid folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/SLiM/ph_results_sigmoid; date
fi

#--------------------------------------------------------------------------------

# Guide file 
GUIDE_FILE=$WORKING_FOLDER/guide_files/slim_ph_guide_file_morevars.txt
...
#scal=`awk -F "\t" '{print $2}' $GUIDE_FILE | sed "${SLURM_ARRAY_TASK_ID}q;d"`
#asym=`awk -F "\t" '{print $3}' $GUIDE_FILE | sed "${SLURM_ARRAY_TASK_ID}q;d"`
m=`awk -F "\t" '{print $4}' $GUIDE_FILE | sed "${SLURM_ARRAY_TASK_ID}q;d"`
N=`awk -F "\t" '{print $5}' $GUIDE_FILE | sed "${SLURM_ARRAY_TASK_ID}q;d"`
echo "Threshold:"${thresh} "scal:"${scal} "asym:"${asym} "m:"${m} "N:" ${N}
