...
file_names = as.list(dir(path = 'data/processed/SLiM/Mcali_results_v5/', pattern = "McaliclineAFs.*"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0('data/processed/SLiM/Mcali_results_v5/', x))))
...
sim_data <- foreach(i=file_names_v, .combine="rbind", .errorhandling = "remove")%do%{  
    ...
    tmp <- read.table(i) %>% 
            mutate(file_name = i) %>% 
            mutate(file_name = str_remove(file_name, pattern = "data/processed/SLiM/Mcali_results_v5/McaliclineAFs.")) %>% 
...
# Save output
save(sim_data, file = "data/processed/SLiM/Mcali_ABC/sim_data_v5.Rdata")