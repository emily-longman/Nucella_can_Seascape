...
# Graph posteriors
pdf("output/figures/SLiM/ph_ABC/pH_posteriors_morvars7.pdf", width = 5, height = 5)
...
# Graph sim
pdf("output/figures/SLiM/ph_ABC/sim_AFs_best_morevars7.pdf", width = 5, height = 5)
...
# Graph sim
pdf("output/figures/SLiM/ph_ABC/sim_AFs_best_repmeans_morevars7.pdf", width = 5, height = 5)
...
sim_AFs_test <- sim_Data.melt %>%
  filter(thresh == 7.9865, k == 0.1, m == 0.001) %>%
  left_join(dplyr::select(ecovars, Site, sim_eq, ph_mean))
# Make Site factor
