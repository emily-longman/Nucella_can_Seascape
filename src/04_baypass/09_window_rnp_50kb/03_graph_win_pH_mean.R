...
path <- paste("data/processed/baypass/window_summary/window_50kb_chunk_analysis_ph_mean/")
file_names = as.list(dir(path = path, pattern = "window_chunks_*"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0(paste("data/processed/baypass/window_summary/window_50kb_chunk_analysis_ph_mean/"), x))))
...
# Save merged data
save(win.out.order, file = "data/processed/baypass/window_summary/window_50kb_analysis_ph_mean.RData")
...
# Graph rnp geompoint
pdf("output/figures/baypass/window_summary/baypass_window_50kb_ph_mean_rnpPOD_geompoint.pdf", width = 12, height = 6)
...
# Graph rnp geomline
pdf("output/figures/baypass/window_summary/baypass_window_50kb_ph_mean_rnpPOD_geomline.pdf", width = 12, height = 6)
ggplot(win.out.order, aes(y=-log10(rnp.binom.POD), x=chr.unique)) + 
