...
path <- paste("data/processed/baypass/window_summary/window_50kb_chunk_analysis_Mcali_IntThk/")
file_names = as.list(dir(path = path, pattern = "window_chunks_*"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0(paste("data/processed/baypass/window_summary/window_50kb_chunk_analysis_Mcali_IntThk/"), x))))
...
# Save merged data
save(win.out.order, file = "data/processed/baypass/window_summary/window_50kb_analysis_Mcali_IntThk.RData")
...
# Graph rnp geompoint
pdf("output/figures/baypass/window_summary/baypass_window_50kb_Mcali_IntThk_rnpPOD_geompoint.pdf", width = 12, height = 6)
...
# Graph rnp geomline
pdf("output/figures/baypass/window_summary/baypass_window_50kb_Mcali_IntThk_rnpPOD_geomline.pdf", width = 12, height = 6)
ggplot(win.out.order, aes(y=-log10(rnp.binom.POD), x=chr.unique)) + 
  geom_line( ) + geom_hline(yintercept=-log10(pr.i), col="red", linetype="dashed") +
  theme_bw(base_size=26) + theme(legend.position = "none", axis.text.x = element_blank(), axis.ticks.x = element_blank()) 
dev.off()
pdf("output/figures/baypass/window_summary/baypass_window_50kb_Mcali_IntThk_rnpPOD_geomline_wider.pdf", width = 12, height = 3)
ggplot(win.out.order, aes(y=-log10(rnp.binom.POD), x=chr.unique)) + 
  geom_line( ) + geom_hline(yintercept=-log10(pr.i), col="red", linetype="dashed") +
  theme_bw(base_size=26) + theme(legend.position = "none", axis.text.x = element_blank(), axis.ticks.x = element_blank()) 
dev.off()
...
# Save outliers
write.csv(win.out.order.outliers, "data/processed/baypass/window_summary/window_analysis_50kb_Mcali_IntThk_outliers.csv", row.names=FALSE)
