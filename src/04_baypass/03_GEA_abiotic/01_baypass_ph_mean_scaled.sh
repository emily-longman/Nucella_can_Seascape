...
# Name this job
#SBATCH --job-name=baypass_ph_mean_scaled
...
# Generate folder
if [ -d "ph_mean_scaled" ]
then echo "Working ph_mean_scaled folder exist"; echo "Let's move on."; date
else echo "Working ph_mean_scaled folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/baypass/abiotic/ph_mean_scaled; date
fi

#--------------------------------------------------------------------------------

# Change directory 
cd $WORKING_FOLDER/data/processed/baypass/abiotic/ph_mean_scaled
...
-d0yij 4 \
-outprefix NC_abiotic_ph_mean_scaled_run${SLURM_ARRAY_TASK_ID} \
-nthreads 20
