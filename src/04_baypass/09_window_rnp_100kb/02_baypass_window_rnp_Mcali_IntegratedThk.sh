...
if [ -d "window_100kb_chunk_analysis_Mcali_IntThk" ]
then echo "Working window_100kb_chunk_analysis_Mcali_IntThk folder exist"; echo "Let's move on."; date
else echo "Working window_100kb_chunk_analysis_Mcali_IntThk folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/baypass/window_summary/window_100kb_chunk_analysis_Mcali_IntThk; date
fi

#--------------------------------------------------------------------------------

# Run R script
Rscript --vanilla $WORKING_FOLDER/src/04_baypass/09_window_rnp/02_baypass_window_rnp_Mcali_IntegratedThk.R "${SLURM_ARRAY_TASK_ID}"
