...
path <- paste("data/processed/baypass/window_summary/window_100kb_chunk_analysis_Mcali_IntThk_17pop/")
file_names = as.list(dir(path = path, pattern = "window_chunks_*"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0(paste("data/processed/baypass/window_summary/window_100kb_chunk_analysis_Mcali_IntThk_17pop/"), x))))
...
# Save merged data
save(win.out.order, file = "data/processed/baypass/window_summary/window_analysis_Mcali_IntThk_17pop.RData")
load("data/processed/baypass/window_summary/window_analysis_Mcali_IntThk_17pop.RData")
...
# Create unique Chromosome number (total 4769)
...
# Graph rnp geompoint
pdf("output/figures/baypass/window_summary/baypass_window_Mcali_IntThk_17pop_rnpPOD_geompoint.pdf", width = 12, height = 6)
...
# Graph rnp geomline
pdf("output/figures/baypass/window_summary/baypass_window_Mcali_IntThk_17pop_rnpPOD_geomline.pdf", width = 12, height = 6)
ggplot(win.out.order, aes(y=-log10(rnp.binom.POD), x=chr.unique)) + 
  geom_line( ) + geom_hline(yintercept=-log10(pr.i), col="red", linetype="dashed") +
  theme_bw(base_size=24) + theme(legend.position = "none")
dev.off()
pdf("output/figures/baypass/window_summary/baypass_window_Mcali_IntThk_17pop_rnpPOD_geomline_wider.pdf", width = 12, height = 3)
ggplot(win.out.order, aes(y=-log10(rnp.binom.POD), x=chr.unique)) + 
  geom_line( ) + geom_hline(yintercept=-log10(pr.i), col="red", linetype="dashed") +
  theme_bw(base_size=24) + theme(legend.position = "none")
dev.off()

pdf("output/figures/baypass/window_summary/baypass_window_Mcali_IntThk_17pop_rnpPOD_geomline_bywindows.pdf", width = 12, height = 3)
...
# Save outliers
write.csv(win.out.order.outliers, "data/processed/baypass/window_summary/window_100kb_analysis_Mcali_IntThk_17pop_outliers.csv", row.names=FALSE)

# ================================================================================== #

# Identify SNPs in the outlier windows
outlier.win.SNPs <- foreach(win.i=unique(win.out.order.outliers$win), .combine="rbind", .errorhandling="remove")%do%{
    ...
    baypass.tmp <- bf.McaliIntThk.mean.sum %>% filter(
        bf.McaliIntThk.mean.sum$chr == win.tmp$chr & 
        bf.McaliIntThk.mean.sum$pos >= win.tmp$pos_min &
        bf.McaliIntThk.mean.sum$pos <= win.tmp$pos_max)
}
...
# Save outlier SNPs - (73,533)
write.csv(outlier.win.SNPs, "data/processed/baypass/window_summary/window_100kb_Mcali_IntThk_17pop_outlier_SNPs.csv", row.names=FALSE)
...
# Write output
write.csv(outlier.win.SNPs.annotated, "data/processed/baypass/window_summary/outlier.win.SNPs.annotated.McaliIntThk.17pop.csv", row.names = F, quote = F)
