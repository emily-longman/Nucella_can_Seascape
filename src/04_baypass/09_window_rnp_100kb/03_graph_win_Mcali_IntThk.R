...
path <- paste("data/processed/baypass/window_summary/window_100kb_chunk_analysis_Mcali_IntThk/")
file_names = as.list(dir(path = path, pattern = "window_chunks_*"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0(paste("data/processed/baypass/window_summary/window_100kb_chunk_analysis_Mcali_IntThk/"), x))))
...
# Save merged data
save(win.out.order, file = "data/processed/baypass/window_summary/window_analysis_Mcali_IntThk.RData")
load("data/processed/baypass/window_summary/window_analysis_Mcali_IntThk.RData")
...
ggplot(win.out.order, aes(y=-log10(rnp.binom.POD), x=win)) + 
  ...
  theme_bw(base_size=24) + theme(legend.position = "none")
dev.off()
pdf("output/figures/baypass/window_summary/baypass_window_Mcali_IntThk_rnpPOD_geomline_bywindows_alt.pdf", width = 18, height = 4)
ggplot(win.out.order, aes(y=-log10(rnp.binom.POD), x=win)) + ylab("Enrichment") + xlab("Window (100kb)") +
  ...
  theme_bw(base_size=30) + theme(legend.position = "none") #+ theme(plot.margin = margin(t = 40, r = 30, b = 20, l = 20, unit = "pt"))
dev.off()
pdf("output/figures/baypass/window_summary/baypass_window_Mcali_IntThk_rnpPOD_geomline_bywindows_alt2.pdf", width = 14, height = 3.7)
ggplot(win.out.order, aes(y=-log10(rnp.binom.POD), x=win)) + ylab("Enrichment") + xlab("Window (100kb)") +
  geom_line(linewidth=1.5) + geom_hline(yintercept=-log10(pr.i), col="red", linetype="dashed", linewidth=1.5) +
...
# Save outliers
write.csv(win.out.order.outliers, "data/processed/baypass/window_summary/window_100kb_analysis_Mcali_IntThk_outliers.csv", row.names=FALSE)
...
# Save outlier SNPs - (32,575)
write.csv(outlier.win.SNPs, "data/processed/baypass/window_summary/window_100kb_Mcali_IntThk_outlier_SNPs.csv", row.names=FALSE)
...
# Write output
write.csv(outlier.win.SNPs.annotated, "data/processed/baypass/window_summary/outlier.win.SNPs.annotated.McaliIntThk.csv", row.names = F, quote = F)
