# Use poolfstat to create PODs for the 18 pop subset
...
pi.beta.coef <- read.table("data/processed/baypass/omega_subset18pop/NC_subset18pop_baypass_summary_beta_params.out", h=T)$Mean

# Omega file
omega <- as.matrix(read.table("data/processed/baypass/omega_subset18pop/NC_subset18pop_baypass_mat_omega.out"))
...
foreach(i=1:5, .errorhandling="remove")%do%{
    suffix <- paste("POD8M_subset18pop.", i, sep="")
    simulate.baypass(omega.mat=omega, nsnp = pooldata.subset.18pop@nsnp, beta.pi=pi.beta.coef, sample.size=pooldata.subset.18pop@poolsizes, suffix=suffix)
}
