...
# Load POD files - M trossulus Mean

# Create list of file names
file_names = as.list(dir(path = 'data/processed/baypass/biotic/Mtross_mean_POD/', pattern = "*summary_betai_reg.out"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0('data/processed/baypass/biotic/Mtross_mean_POD/', x))))
...
bf.POD <- foreach(w=file_names_v, .combine = rbind)%do%{  
    ...
    tmp = fread(w, header=T)
    # Add column with identifier
    tmp <- tmp %>% mutate(run = w) %>% mutate(run = str_remove(run, pattern = "data/processed/baypass/biotic/Mtross_mean_POD/NC_biotic_Mtross_mean_POD_run*"))
    # Remove end of chunk name
    tmp <- tmp %>% mutate(run = str_remove(run, pattern = "_summary_betai_reg.out"))
    #Return
    return(tmp)
...
# Graph BF of one POD to see distribution with threshold
pdf("output/figures/baypass/baypass_Mtross_mean_BF_POD1.pdf", width = 8, height = 8)
ggplot(bf.POD[which(bf.POD$run==1),], aes(y=bf_db, x=MRK)) + 
  labs(x = "Position", y = "BF (in dB)") +
  geom_point(alpha=0.7) + 
  theme_classic(base_size = 20) + 
...
# Save
save(bf.POD.thr, file="data/processed/baypass/biotic/Mtross_mean_POD_thr.Rdata")
...
# Create list of file names
file_names = as.list(dir(path = 'data/processed/baypass/abiotic/ph_mean_scaled_POD/', pattern = "*summary_betai_reg.out"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0('data/processed/baypass/abiotic/ph_mean_scaled_POD/', x))))
...
bf.POD <- foreach(w=file_names_v, .combine = rbind)%do%{  
    ...
    tmp = fread(w, header=T)
    # Add column with identifier
    tmp <- tmp %>% mutate(run = w) %>% mutate(run = str_remove(run, pattern = "data/processed/baypass/abiotic/ph_mean_scaled_POD/NC_abiotic_ph_mean_POD_run*"))
    # Remove end of chunk name
    tmp <- tmp %>% mutate(run = str_remove(run, pattern = "_summary_betai_reg.out"))
    #Return
    return(tmp)
...
# Graph BF of one POD to see distribution with threshold
pdf("output/figures/baypass/baypass_ph_mean_scaled_BF_POD1.pdf", width = 8, height = 8)
ggplot(bf.POD[which(bf.POD$run==1),], aes(y=bf_db, x=MRK)) + 
  labs(x = "Position", y = "BF (in dB)") +
  geom_point(alpha=0.7) + 
  theme_classic(base_size = 20) + 
...
# Save
save(bf.POD.thr, file="data/processed/baypass/abiotic/ph_mean_scaled_POD_thr.Rdata")
...
# Create list of file names
file_names = as.list(dir(path = 'data/processed/baypass/biotic/Mcali_IntegratedThk_POD_17pop/', pattern = "*summary_betai_reg.out"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0('data/processed/baypass/biotic/Mcali_IntegratedThk_POD_17pop/', x))))
...
bf.POD <- foreach(w=file_names_v, .combine = rbind)%do%{  
    ...
    tmp = fread(w, header=T)
    # Add column with identifier
    tmp <- tmp %>% mutate(run = w) %>% mutate(run = str_remove(run, pattern = "data/processed/baypass/biotic/Mcali_IntegratedThk_POD_17pop/NC_biotic_Mcali_IntegratedThk_POD_run*"))
    # Remove end of chunk name
    tmp <- tmp %>% mutate(run = str_remove(run, pattern = "_summary_betai_reg.out"))
    #Return
    return(tmp)
...
# Graph BF of one POD to see distribution with threshold
pdf("output/figures/baypass/baypass_Mcali_IntegratedThk_BF_POD1_17pop.pdf", width = 8, height = 8)
ggplot(bf.POD[which(bf.POD$run==1),], aes(y=bf_db, x=MRK)) + 
  labs(x = "Position", y = "BF (in dB)") +
  geom_point(alpha=0.7) + 
  theme_classic(base_size = 20) + 
...
# Save
save(bf.POD.thr, file="data/processed/baypass/biotic/Mcali_IntegratedThk_POD_17pop_thr.Rdata")
