...
# Name this job
#SBATCH --job-name=baypass_GWAS_phenotypic_POD
...
if [ -d "Baypass_GWAS_POD" ]
then echo "Working Baypass_GWAS_POD folder exist"; echo "Let's move on."; date
else echo "Working Baypass_GWAS_POD folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/phenotypic_data/Baypass_GWAS_POD; date
fi

#--------------------------------------------------------------------------------

# Change directory 
cd $WORKING_FOLDER/data/processed/phenotypic_data/Baypass_GWAS_POD

# Run baypass - this will generate the omega file which will be used in the subsequent scripts
$baypass -npop 11 \
-gfile $WORKING_FOLDER/data/processed/phenotypic_data/PODs/G.POD8M.${SLURM_ARRAY_TASK_ID} \
...
-d0yij 4 \
-outprefix NC_pheno_POD_run${SLURM_ARRAY_TASK_ID} \
