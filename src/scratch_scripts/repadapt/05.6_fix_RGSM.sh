...
if [ -d "merge_lanes_fix_RG" ]
then echo "Working merge_lanes_fix_RG folder exist"; echo "Let's move on."; date
else echo "Working merge_lanes_fix_RG folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/repadapt/merge_lanes_fix_RG; date
fi
...
O=$WORKING_FOLDER/data/processed/repadapt/merge_lanes_fix_RG/${i}_sorted_dedup_RG_lanes_merged.bam \
