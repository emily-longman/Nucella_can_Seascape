...
# Create function
process_sims = function(repId, m, thresh, slope_1, slope_2, N){
  #r=1; m=0.001; t=7.96; slope_1=2; slope_1=0; N=2500
...
file_names = as.list(dir(path = 'data/processed/SLiM/ph_results_linear/', pattern = "phclineAFs.*"))
file_names_v = as.vector(unlist(lapply(file_names, function(x) paste0('data/processed/SLiM/ph_results_linear/', x))))
...
sim_data <- foreach(i=file_names_v, .combine="rbind", .errorhandling = "remove")%do%{  
    ...
    tmp <- read.table(i) %>% 
            mutate(file_name = i) %>% 
            mutate(file_name = str_remove(file_name, pattern = "data/processed/SLiM/ph_results_linear/phclineAFs.")) %>% 
            mutate(file_name = str_remove(file_name, pattern = ".txt"))
    # Rename pops
    names(tmp)[1:19] = paste("p", 0:18, sep ="")
    # Separate columns based on parameters
    sim_Data <- separate_wider_delim(tmp, cols = file_name, delim = "_", names = c("repId", "m", "thresh", "slope_1", "slope_2", "N", "state", "sim.cycle"))

    # Reformat
    sim_Data_melt <- sim_Data %>%
            reshape2::melt(id = c("repId","m","thresh","slope_1","slope_2","N","state","sim.cycle"), 
    ...
    slope_1_i = unique(sim_Data_melt$slope_1)
    slope_2_i = unique(sim_Data_melt$slope_2)
    N_i = unique(sim_Data_melt$N)

    # Run function
    process_sims(repId_i, m_i, thresh_i, slope_1_i, slope_2_i, N_i) %>%
            mutate(repId = repId_i, m = m_i, thresh = thresh_i, slope_1 = slope_1_i, slope_2 = slope_2_i, N = N_i)
...
# Save output
save(sim_data, file = "data/processed/SLiM/ph_ABC/sim_data_linear.Rdata")
