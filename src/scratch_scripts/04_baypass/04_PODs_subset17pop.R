...
# Subset pooldata - cut ARA (4) and HZD (9)
...
pi.beta.coef <- read.table("data/processed/baypass/omega_subset17pop/NC_subset17pop_baypass_summary_beta_params.out", h=T)$Mean

# Omega file
omega <- as.matrix(read.table("data/processed/baypass/omega_subset17pop/NC_subset17pop_baypass_mat_omega.out"))
...
foreach(i=1:5, .errorhandling="remove")%do%{
    suffix <- paste("POD8M_subset17pop.", i, sep="")
    simulate.baypass(omega.mat=omega, nsnp = pooldata.subset.17pop@nsnp, beta.pi=pi.beta.coef, sample.size=pooldata.subset.17pop@poolsizes, suffix=suffix)
}
