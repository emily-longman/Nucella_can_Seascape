...
#SBATCH --time=30:00:00 #30:00:00
#SBATCH --cpus-per-task=25 #10
#SBATCH --mem-per-cpu=120G #30G
...
# Map read
apptainer run $repadapt_bwa bwa mem -t 25 $WORKING_FOLDER/data/processed/repadapt/genome/N.canaliculata_assembly.fasta.softmasked.fa \
...
apptainer run $repadapt_samtools samtools view -Sb -q 10 --threads 25 $WORKING_FOLDER/data/processed/repadapt/bwa_output/$i\.sam > $WORKING_FOLDER/data/processed/repadapt/bwa_output/$i\.bam
rm $WORKING_FOLDER/data/processed/repadapt/bwa_output/$i\.sam

# Sort bam file (remove unsorted bam file)
apptainer run $repadapt_samtools samtools sort --threads 25 $WORKING_FOLDER/data/processed/repadapt/bwa_output/$i\.bam > $WORKING_FOLDER/data/processed/repadapt/bwa_output/$i\_sorted.bam
