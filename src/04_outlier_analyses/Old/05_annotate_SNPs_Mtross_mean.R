...
Mtross.mean.outliers <- read.csv("data/processed/outlier_analyses/Mtross.mean.outliers.csv", header=T)

#--------------------------------------------------------------------------------

# Extract annotation data for each SNP of interest

annotation <- foreach(i=1:dim(Mtross.mean.outliers)[1], .combine = "rbind", .errorhandling = "remove")%do%{

  message(i)
  # Reset filter
  seqResetFilter(genofile)
  # Extract SNP_id for SNP i
  tmp.i = Mtross.mean.outliers[i,]$SNP_id
...
Mtross.mean.annotated.SNPs <- left_join(Mtross.mean.outliers, annotation, by = join_by(SNP_id))

# Write output
write.csv(Mtross.mean.annotated.SNPs, "data/processed/outlier_analyses/Mtross.mean.annotated.SNPs.csv", row.names = F, quote = F)
