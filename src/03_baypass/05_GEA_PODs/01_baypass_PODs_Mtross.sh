...
# Name this job
#SBATCH --job-name=baypass_PODs_Mtross_mean
...
# Generate folder
if [ -d "Mtross_mean_POD" ]
then echo "Working Mtross_mean_POD folder exist"; echo "Let's move on."; date
else echo "Working Mtross_mean_POD folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/baypass/biotic/Mtross_mean_POD; date
fi

#--------------------------------------------------------------------------------

# Change directory 
cd $WORKING_FOLDER/data/processed/baypass/biotic/Mtross_mean_POD
...
-d0yij 4 \
-outprefix NC_biotic_Mtross_mean_POD_run${SLURM_ARRAY_TASK_ID} \
-nthreads 20
