...
# Name this job
#SBATCH --job-name=glms_window_rnp_Mtross_mean
...
# Submit job array
#SBATCH --array=596,599,601 #501-995%50 #1-500%100
...
if [ -d "glms_window_chunk_analysis_Mtross_mean" ]
then echo "Working glms_window_chunk_analysis_Mtross_mean folder exist"; echo "Let's move on."; date
else echo "Working glms_window_chunk_analysis_Mtross_mean folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/GEA/glms/glms_window_summary/glms_window_chunk_analysis_Mtross_mean; date
fi
...
Rscript --vanilla $WORKING_FOLDER/src/02_GEA_glms/03_glms_window_rnp/02_glm_window_rnp_Mtross_mean.R "${SLURM_ARRAY_TASK_ID}"
