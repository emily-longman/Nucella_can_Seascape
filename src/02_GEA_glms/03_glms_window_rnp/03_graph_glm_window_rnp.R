...
# Check structure - 1244017 windows
str(win.out)
...
# Extract outliers - 132
win.ph.mean.outliers <- win.ph.mean.summary[which(win.ph.mean.summary$beat.perm == "TRUE"),]

# Write outliers to csv
write.csv(win.ph.mean.outliers, "data/processed/GEA/glms/glms_window_summary/win.ph.mean.outliers.csv", row.names=FALSE)
...
ggplot() +
  geom_line(data=win.ph.mean.summary, aes(y=-log10(uci), x=chr.unique.x), color = "#a1c8cf") + ylim(0,250) +
  geom_point(data=win.ph.mean.outliers, aes(y=-log10(rnp.binom.p.0.05), x=chr.unique.x)) +
  xlab("Position") + ylab("-log10(rnp.binom.p)") +
  theme_bw(base_size=18)
dev.off()
...
# Check structure - 1207467 windows
str(win.out)
...
# Summarize M trossulus model
win.Mtross.mean <- win.out %>%
  mutate(data_type = case_when(perm == 0 ~ "real",
                               perm != 0 ~ "perm")) %>%
  group_by(data_type, win, chr, pos_mean) %>%
  summarise(uci = quantile(rnp.binom.p.0.05, 0.01)) %>%
  mutate(model = "M.tross_m")
...
# Extract perm uci
win.Mtross.mean.perm.uci <- win.Mtross.mean %>% filter(data_type == "perm") %>% data.frame()

# Join real data with perm uci
win.Mtross.mean.summary <- left_join(win.out.real, win.Mtross.mean.perm.uci, by = "win")

# Identify which windows beat uci of permutations
win.Mtross.mean.summary <- win.Mtross.mean.summary %>% mutate(beat.perm = (-log10(rnp.binom.p.0.05) > -log10(uci)))

# Extract outliers - 57
win.Mtross.mean.outliers <- win.Mtross.mean.summary[which(win.Mtross.mean.summary$beat.perm == "TRUE"),]

# Write outliers to csv
write.csv(win.Mtross.mean.outliers, "data/processed/GEA/glms/glms_window_summary/win.Mtross.mean.outliers.csv", row.names=FALSE)

# ================================================================================== #

# Graph uci of perm and outliers of real
pdf("output/figures/GEA/glms/glms_window_summary/glm_window_Mtross_mean_perm_99uci_real.pdf", width = 14, height = 8)
ggplot() +
  geom_line(data=win.Mtross.mean.summary, aes(y=-log10(uci), x=chr.unique.x), color = "#a1c8cf") + ylim(0,250) +
  geom_point(data=win.Mtross.mean.outliers, aes(y=-log10(rnp.binom.p.0.05), x=chr.unique.x)) +
  xlab("Position") + ylab("-log10(rnp.binom.p)") +
  theme_bw(base_size=18)
dev.off()
